...
from Unet.GeoLoss import GeometricErrorLoss

import pytorch_ssim
import numpy as np

class GeoSRTrainer(object):
    def __init__(self, config, training_loader, testing_loader):
        super(GeoSRTrainer, self).__init__()
    ...
    def build_model(self):
        ...
        self.criterion_geo=GeometricErrorLoss()
    ...
    def train(self):
        ...
        for batch_num, (data, target) in enumerate(self.training_loader):
            ...
            loss_geo=self.criterion_geo(prediction, target)

            loss_ssim=1-pytorch_ssim.ssim(prediction, target)
            #loss = loss1+loss2#!!!!!!!!!
            loss = loss1+0.1*loss_ssim+0.1*loss_geo
            '''
            print("\nloss1")
